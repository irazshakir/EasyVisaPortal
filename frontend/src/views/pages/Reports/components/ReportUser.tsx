...
import { reportsApi, UserStatusReportResponse, UserStatusReportParams } from '../api/apiReports';
import ReportFilter from './ReportFilter';

const { THead, TBody, Tr, Th, Td } = Table;

const ReportUser = () => {
    const [data, setData] = useState<UserStatusReportResponse | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [isFilterOpen, setIsFilterOpen] = useState(false);
    const [filterParams, setFilterParams] = useState<UserStatusReportParams>({ range: 'month' });

    const fetchReport = useCallback(async (params: UserStatusReportParams) => {
        setLoading(true);
        setError(null);
        try {
            const response = await reportsApi.getUserStatusReport(params);
            setData(response);
        } catch (err) {
    ...
    const handleFilterApply = (range: { from: Date | null, to: Date | null, quick: string | null }) => {
        if (range.quick) {
            setFilterParams({ range: range.quick as UserStatusReportParams['range'] });
        ...
        <Card>
            <div className="p-4">
                <div className="flex items-center justify-between mb-4">
                    <h4 className="font-semibold">User x Status Report</h4>
                    <Button
                        variant="plain"
                        size="sm"
                ...
                ) : data && data.users.length && data.statuses.length ? (
                    <Table>
                        <THead>
                            <Tr>
                                <Th>User</Th>
                        ...
                        <TBody>
                            {data.matrix.map(row => {
                                // Get new leads for this user from API response
                                const newLeads = data.new_leads?.find(nl => nl.user_id === row.user_id)?.new_leads ?? 0;
                                const desiredOrder = [
                                    'New Query',
                                    'Not Qualified',
                                    'Qualified',
                                ...
                                return (
                                    <Tr key={row.user_id}>
                                        ...
                                        {orderedCounts.map(cell => {
                                            ...
                                            return (
                                                <Td key={cell.status_id} className={cellClass}>{cell.count}</Td>
                                            );
                                        })}
                                    </Tr>
                                );
