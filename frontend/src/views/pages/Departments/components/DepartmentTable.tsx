...
const DepartmentTable = () => {
    ...
    const fetchDepartments = async () => {
        ...
        try {
            ...
            setTotalDepartments(response.count)
        } catch (error) {
            toast.push(
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                                                    <HiOutlineOfficeBuilding className="text-blue-600 text-sm" />
                                                </div>
                                                <div>
                                                    <div className="font-semibold">{department.department_name}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className={`px-2 py-1 rounded-full text-xs inline-block ${getStatusColor(department.is_active)}`}>
                                                {department.is_active ? 'Active' : 'Inactive'}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            {dayjs(department.created_at).format('DD/MM/YYYY')}
                                        </td>
                                        <td className="px-4 py-3">
                                            {dayjs(department.updated_at).format('DD/MM/YYYY')}
                                        </td>
                                        <td className="px-4 py-3">
